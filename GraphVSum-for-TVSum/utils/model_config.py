...
from models.GraphVsum.story_sum import StorySum as GraphVSum
...
def get_model(config: dict)->Callable:
    ...
    if config["ours_model"]:
        kwargs = {"modality": config["modality"],
                ...
                "shot_constraint": config.get("shot_constraint", False),
                "target_graph_density": config.get("target_graph_density"),
                "zeta_v": config.get("zeta_v", 0.95),
                "zeta_t": config.get("zeta_t", 0.97),
