...
class ParseEPS(object):
    ...
    def convert2Yamlable(dct: Dict) -> Dict:
        dct["train"] = [ep.as_posix() if hasattr(ep, "as_posix") else str(ep) for ep in dct["train"]]
        dct["val"] = [ep.as_posix() if hasattr(ep, "as_posix") else str(ep) for ep in dct["val"]]
        dct["test"] = [ep.as_posix() if hasattr(ep, "as_posix") else str(ep) for ep in dct["test"]]
        dct["human"] = [ep.as_posix() if hasattr(ep, "as_posix") else str(ep) for ep in dct["human"]]
        return dct
