...
class Trainer(object):
    ...
    def __init__(self, cfg: DictConfig) -> None:
        ...
        if cfg.get("loss_type", "bce") == "focal":
            self.criterion = FocalLoss(alpha=cfg.get("focal_alpha", 0.25),
        ...
        if cfg['mode'] == 'training':
            ...
            if cfg.get("optimizer", "adamw") == "adam":
                self.optimizer = optim.Adam(model.parameters(), lr=cfg["lr"], weight_decay=cfg["weight_decay"], amsgrad=cfg["amsgrad"])
            else:
            ...
            elif cfg['lr_scheduler'] in ['none', None]:
                self.scheduler = None
            else:
    ...
    def prepare_data(self, mode:str) -> DataLoader:
        ...
        common_params = {'vary_window_size': self.cfg['vary_window_size'],
                         ...
                         'limit_samples': self.cfg.get('limit_samples'),
    ...
    def transformANDforward(self, data_batch: Dict, edge_dict, alignment_ref = None, epoch = 0)->Tuple:
        ...
        if self.modality == "both":
            ...
            vid_feat_dict = feat_dict[0] if isinstance(feat_dict, (list, tuple)) else feat_dict
            dia_feat_dict = None
            dia_targets = None
            dia_boolean_mask = None
        elif self.modality == "dia":
            dia_feat_dict = feat_dict[1] if isinstance(feat_dict, (list, tuple)) else feat_dict
            vid_feat_dict = None
            vid_targets = None
            vid_boolean_mask = None
        else:
        ...
        if self.modality != "vid":
            dia_feat_dict = {
                k: v.to(self.device).bool() if "mask" in k else v.to(self.device).float()
                for k, v in dia_feat_dict.items()
            }
        ...
        for epoch in range(self.epochs):
            ...
            for _, data_batch in enumerate(tqdm(train_dl, disable=self.wandb_logging)):
                ...
                if self.scheduler is not None:
                    self.scheduler.step() # CyclicLR/OneCycleLR: called on every batch
            ...
            if self.wandb_logging:
                ...
                if self.modality == "both":
                    ...
                    if self.eval_test:
                        ...
                        if self.modality == "vid":
                            if test_scores[0] > self.best_test_vid_AP:
                                self.best_test_vid_AP = test_scores[0]
                            if test_scores[1] > self.best_test_vid_F1:
                                self.best_test_vid_F1 = test_scores[1]
                        elif self.modality == "dia":
                            if test_scores[0] > self.best_test_dia_AP:
                                self.best_test_dia_AP = test_scores[0]
                            if test_scores[1] > self.best_test_dia_F1:
                                self.best_test_dia_F1 = test_scores[1]
...
def main(config: DictConfig):
    ...
    if os.path.isfile(split_type_path):    
        ...
        if isinstance(episode_config, list):
            split_ids = OmegaConf.to_container(config["split_id"], resolve=True) if isinstance(config["split_id"], ListConfig) else config["split_id"]
            split_idx = split_ids[0] if isinstance(split_ids, (list, tuple)) else split_ids
            fold = episode_config[int(split_idx)]
            split_dict = {
                "train": [Path(key).name for key in fold["train_keys"]],
                "val": [Path(key).name for key in fold["test_keys"]],
                "test": [Path(key).name for key in fold["test_keys"]],
                "human": [Path(key).name for key in fold["test_keys"]],
            }
        elif all(key in episode_config for key in ["train", "val", "test"]):
            split_dict = {key: episode_config[key] for key in ["train", "val", "test", "human"] if key in episode_config}
        else:
...
class calc_loss_class(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        modality = cfg['modality']
        device = torch.device(f"cuda:{cfg['gpus'][0]}" if torch.cuda.is_available() and len(cfg["gpus"]) >= 1 else "cpu")
